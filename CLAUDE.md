...
This repository contains **Animaxxing skills** in three families. **Framework skills** (`gsap-<framework>`) cover page transition and component lifecycle guidance for GSAP in specific frameworks; each sits above the official [GSAP skills](https://github.com/greensock/gsap-skills), which cover the GSAP API itself. **Motion skills** (`motion-<name>`) supply reusable vanilla TypeScript and GSAP effects, technical requirements, and effect checks without prescribing a brand. **Aesthetic skills** (`aesthetic-<name>`) carry one art direction: design tokens, typography and layout grammar, and a curated selection and configuration of motion recipes. The framework owns when effects run; motion owns their implementation; the aesthetic owns the look and selects the treatment. When editing or adding skills, follow these rules.
...
- Every framework skill uses the same lifecycle: **mount → initial state → intro → settled → outro → end state → unmount**. Reuse the wording from `gsap-nextjs`. Motion skills name that lifecycle as the framework skill's and map their builders onto its phases. Aesthetics select the treatment for those phases without owning execution timing.
...
- Neither motion nor aesthetic skills own lifecycle: no routing, mounting, cleanup timing, or navigation rules. Motion recipes expose builders and cleanup handles for the framework controller to call; aesthetics select and configure them. Name the framework skill family as the lifecycle owner. Tailwind class strings need a plain-CSS equivalent.
...
- Motion skill layout: `skills/motion-<name>/SKILL.md`, `agents/openai.yaml`, `references/motion-vocabulary.md`, `text-stability.md` when relevant, `verification.md`, and `references/recipes/*.md`, one self-contained module per recipe with its lifecycle contract stated at the top.
- Aesthetic skill layout: `skills/aesthetic-<name>/SKILL.md`, `agents/openai.yaml`, `references/tokens.md`, `typography-and-layout.md`, `motion-vocabulary.md` for art direction and surface choices, and `verification.md` for design checks.
- When adding a new skill: create `skills/<skill-name>/SKILL.md`, then update `skills/llms.txt` and the README "Skills" and "Structure" sections. For a framework skill, add a matching framework directory to the animaxxing-skills-test repository (starter, TASK.md, reference app, specs) so the skill can be verified. For a motion or aesthetic skill, record how it is verified in its `references/verification.md` and point at the demo that exercises it until the test repository grows dedicated suites. Motion verification also checks reuse without the demo's branding.
