---
name: animaxxing
description: "Build GSAP text, scroll, pointer, and particle effects without restyling: split-text entrances, scattering headlines, speak-in, letter waves, scroll reveals, scrubbed statements, parallax, pinned scenes, horizontal runs, magnetic buttons, tilt cards, cursor followers, drag-and-throw tracks, particle controls, and blast-off exits. Use for Animaxxing motion or hover effects that stick on touch. Pair with the matching GSAP framework skill for lifecycle timing. Not for branding, layout redesign, routing, or isolated GSAP API questions."
license: MIT
metadata:
  short-description: Reusable GSAP text, scroll, pointer, and particle motion for any brand
---
...
- Check installed GSAP docs/types. SplitText recipes require 3.13+ (`SplitText.create`, `smartWrap`, `mask`, `aria`); scroll recipes need ScrollTrigger; drag tracks need Draggable and InertiaPlugin. Register only the plugins used. Use the official GSAP skills for API details as needed.
