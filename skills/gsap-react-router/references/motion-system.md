...
- StrictMode double-runs the effect path, not the handler path. A layout-effect play needs a ref guard that clears the pending state after the first play. A revalidation or a fetcher re-renders in place with no handler to capture from; treat that content as a component entrance, not a Flip.
