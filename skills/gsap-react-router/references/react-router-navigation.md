...
- `POP` is also the navigation type of every document's first location. Tell a real history arrival apart with `location.key !== "default"`, or, on a document's first render, with `performance.getEntriesByType("navigation")[0]?.type === "back_forward"`.
...
Follow [Smooth scrolling](smooth-scroll.md) for the contract; the `animaxxing` skill's `references/recipes/smooth-scroll.md` supplies the controls. React Router specifics:

- Create the scroller in the root route component, which outlives every client navigation, from `useGSAP` or a layout effect with no dependencies, and destroy it in that cleanup. Keep the handle in a ref and share it through context, never module scope. StrictMode and Vite's Fast Refresh both rerun that effect; the cleanup's `destroy()` runs before the second create, so the pair leaves one scroller. In framework mode the root module is evaluated on the server, so the create stays inside the effect. A ScrollSmoother wrapper goes around `<Outlet>` in the root, with the chrome, the route cover, and any curtain outside it.
- **Outro.** `stop()` when the blocker blocks or the wrapper intercepts, before the outro timeline starts.
- **End state and swap.** Still stopped through `proceed()`, the loaders, and the commit. `<ScrollRestoration>` [framework, data] sets `history.scrollRestoration` to `manual`, saves positions per `location.key` or `getKey`, and applies its result in a layout effect at commit: the saved position on `POP`, the hash target, nothing with `preventScrollReset`, otherwise the top. Leave it on.
- **New page mounted.** From the boundary's microtask after the commit, the same one that steps the page from initial to intro, `scrollTo(window.scrollY, { immediate: true })` so the engine adopts what the router chose. The page's own layout effect shares the commit with the restore and can run before it.
- **Intro built.** `resize()` once the page's triggers exist, and again at settled if the intro changed height above a trigger.
- **Settled.** `start()`. Also `start()` wherever the lock releases without an intro: `reset()`, a `POP` that kills an outro, the ready timeout.
- Declarative mode has no `<ScrollRestoration>`: the router scrolls nowhere and the browser keeps `auto` restoration. Jump to the top yourself after a `PUSH` commits and sync to `window.scrollY` after a `POP`.
- Same-page hash links stay native and instant; `<ScrollRestoration>` scrolls the hash target into view at commit before it honors `preventScrollReset`. To ease one, prevent default and `scrollTo(hash)` without navigating, then move focus to the target.
...
- A curtain is the one cover allowed to span the chrome. Render its panels in the root route component after `<Outlet>` and outside any ScrollSmoother wrapper, build `curtain()` once in the boundary's setup, and let it replace the route-area cover for the navigations that use it. The recipe's CSS keeps the panels hidden, so server HTML never paints them.
- The blocker receives locations, not elements, so a link opts into the curtain through its `onClick`: record a request keyed by its destination with the boundary, and let the blocker read it. Build `cover()` on the shell's timeline, not inside the page's `useGSAP` context, whose cleanup reverts every child; the page's end state waits for it (`await coverTimeline`). Call `proceed()` from its completion. The loaders, the pending gap the blocker path costs, and the commit all happen under the curtain, with the scroller stopped. `reveal()` starts from the incoming page's ready signal after the new `location.key` commits. A `POP` gets no cover; one that arrives while covered, which the blocker function sees as `historyAction === "POP"`, kills the outro, drops the destination, and reveals once the popped page is ready. A second click during `cover()` retargets the blocker: keep covering and call the chosen `proceed` once. A click during `reveal()` calls `cover()` again. Register the reveal with the boundary's recovery so a timed-out page never stays covered.
- Preloader, framework mode with SSR or `prerender`: decide in the root `Layout`'s inline script from `sessionStorage` and set a root attribute. Server HTML cannot know the session, so render the markup unconditionally in `Layout`, which wraps the app, `HydrateFallback`, and `ErrorBoundary` alike, and show it only under that attribute in CSS. When the attribute is present the boundary builds `preloader(root)`, sets `aria-busy="true"` on the route container, and feeds `progress()` from `document.fonts.ready`, the hero image's `decode()`, and the first route's commit, which a `clientLoader` with `hydrate` delays behind `HydrateFallback`, each bounded by the deadline the same script armed. `finish()` overlaps the first intro while the page waits at initial state. `ssr: false` prerenders the root shell too, so the same rule holds; data and declarative mode paint nothing before React, so the root can render it from a `sessionStorage` read in its first render. The root never remounts on a client navigation, so it never shows again.
- Shared elements: call `captureShared` in the blocker outro before `proceed()`, with no cover and no `viewTransition` on that navigation, or the browser snapshots the element GSAP is about to morph. Keep the handoff in the boundary, keyed by `blocker.location.pathname`; reads must not consume it, since StrictMode runs the page setup twice. Clear it at settle, on `reset()`, on a retarget, and on `POP`. Play `playShared(state, target)` from the ready microtask after the new `location.key` commits, target explicit. A `POP` replays a recorded view transition pair and carries no captured state, so leave the shared element unnamed and give history moves the ordinary intro.
