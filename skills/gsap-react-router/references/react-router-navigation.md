...
- Compose `cover()` into the blocker outro and call `proceed()` from its completion. The loaders, the pending gap the blocker path costs, and the commit all happen under the curtain, with the scroller stopped. `reveal()` starts from the incoming page's ready signal after the new `location.key` commits. A `POP` gets no cover; one that arrives while covered kills the outro, drops the destination, and reveals once the popped page is ready. A second click during `cover()` retargets the blocker: keep covering and call the chosen `proceed` once. A click during `reveal()` calls `cover()` again. Register the reveal with the boundary's recovery so a timed-out page never stays covered.
